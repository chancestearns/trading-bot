    ...
    return Config(
        engine=EngineConfig(
            ...
            data_provider=_make_dataclass(
                DataProviderConfig,
                engine_data.get("data_provider", {})
            ),
            broker=_make_dataclass(
                BrokerConfig,
                engine_data.get("broker", {})
            ),
            strategy=_make_dataclass(
                StrategyConfig,
                engine_data.get("strategy", {})
            ),
            risk=_make_dataclass(
                RiskConfig,
                engine_data.get("risk", {})
            ),
            logging=_make_dataclass(
                LoggingConfig,
                engine_data.get("logging", {})
            ),
        )
    )
...
def _make_dataclass(model_cls, values: Dict[str, Any]):
    ...
    try:
        ...
        raise ConfigValidationError(
            f"Failed to create {model_cls.__name__}: {e}"
        )
...
def mask_secrets(config: Config) -> Dict[str, Any]:
    ...
    def mask_dict(d: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in d.items():
            key_lower = key.lower()
            if any(secret in key_lower for secret in ['key', 'secret', 'password', 'token']):
                result[key] = '***'
