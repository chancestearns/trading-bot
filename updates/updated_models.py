...
class Position:
    ...
    def update(self, fill_quantity: float, fill_price: float) -> None:
        ...
        if abs(new_quantity) < 1e-6:
            self.quantity = 0.0
            self.avg_price = 0.0
        elif (self.quantity > 0) == (new_quantity > 0):
            total_cost = (self.avg_price * abs(self.quantity)) + (fill_price * abs(fill_quantity))
...
class PortfolioState:
    ...
    def net_exposure(self) -> float:
        ...
        return sum(abs(position.quantity * position.avg_price)
                   for position in self.positions.values())

    @property
    def long_exposure(self) -> float:
        """Total long position value."""
        return sum(position.quantity * position.avg_price
                   for position in self.positions.values()
                   if position.is_long)
